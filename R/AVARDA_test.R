AVARDA_test = function(case_path,thresh,dict_path,total_path,pairwise_path,blast_path,out_path,out_name){
  ...
  zeta = foreach(R = 1:(dim(case)[2]-1),.combine=rbind) %dopar%{ #cycle through each patient column by column (goal is so be serialized)
    ...
    rank <- total_calc(case,R,enrich,total,blast,dict) # run the subsetting step given input of the total null probs, sample column, enrichment threshold and hte Virus blast matrix.

    if(is.null(rank) == FALSE){
      sorted_table=pairwise_calc(total,pairwise,rank,dict) # take subset matrix from above and do all reassignments with pairwise and total null probabilities
